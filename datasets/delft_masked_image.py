...
#--------------------mask目标用----------------------------------
import cv2
def apply_mask(img, corners_img):
    # 计算凸包
    hull = cv2.convexHull(corners_img.astype(np.float32))
    hull = hull.astype(np.int32)
    
    # 创建一个空白mask，大小与原图像相同
    mask = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
    
    # 使用OpenCV的cv2.fillPoly()函数来填充多边形
    cv2.fillPoly(mask, [hull], 255)
    
    # 将mask扩展到与原图像相同的形状，以便应用于所有通道
    mask = np.stack([mask, mask, mask], axis=-1)
    
    # 将mask应用于原始图像，将不在mask上的像素点设置为0
    result = img * (mask / 255)
    result = result.astype(np.uint8)
    return result

def apply_mask_with_smooth_edges(img, corners_img, sigma=10):
    hull = cv2.convexHull(corners_img.astype(np.float32))
    polygon = hull.astype(np.int32)
    mask = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
    cv2.fillPoly(mask, [polygon], 255)
    
    ksize = (2 * int(3 * sigma) + 1, 2 * int(3 * sigma) + 1)
    blurred_mask = cv2.GaussianBlur(mask.astype(np.float32), ksize, sigmaX=sigma, sigmaY=sigma)
    
    mask = np.stack([blurred_mask, blurred_mask, blurred_mask], axis=-1) / 255
    result = img * mask
    result = result.astype(np.uint8)
    return result
...
def transform_points(points, trans_matrix): #模仿box转换中center转换的实现，转换points
    # 输入检查，确保points的形状为(N, 3)
    if points.shape[1] != 3:
        raise ValueError("Invalid shape for points. Expected (N, 3), but got {}".format(points.shape))

    # 创建一个表示点的向量，形状为(N, 4)
    points_homogeneous = np.hstack((points, np.ones((points.shape[0], 1))))

    # 将点从一个坐标系转换到另一个坐标系
    transformed_points = (trans_matrix @ points_homogeneous.T).T

    return transformed_points[:, :3]

#---------------------utils end-------------------------------

class DelftImageDataset(base_dataset.BaseDataset):
    ...
    #专注于判断序列开头的box里面有没有点，假如前3帧没有点，第4帧首次出现了点，那之后的不管了，只删除前三帧 
    ...
    #专注于判断box是不是在图像里
    def process_tracklets_V4(self,tracklet_df, track_id_list, x_limit, y_limit, z_limit, min_length):
        """
        根据给定的x和y限制，处理tracklets数据。
        ...
        for idx, row in tracklet_df.iterrows():
            frame = row['frame']
            box = self.get_radarbox_on_camera(row,frame)
            # #统计box信息
            # with open('./%s_all_box_stats.txt'%(str(self.class_names[0])), 'a+') as f:
            #     print(f"{box[0]},{box[1]},{box[2]}",file=f)
            corners = boxes_to_corners_3d(box.reshape(-1,7))
    ...
    def get_image(self,frame_id):
        ...
        frame_loader = FrameDataLoader(kitti_locations=self.DelftLocation,
                             frame_number=img_name)
        return frame_loader.image #H*W*3
    ...
    def get_radarbox_on_camera(self,label: pd.Series,frame) -> np.array:
        # 解压数据
        x = label['x']
        y = label['y']
        z = label['z']
        ...
        transform_matrices = {
            ...
            'lidar': frame_transforms.t_camera_lidar,
            'radar': frame_transforms.t_camera_radar
        } #这个实际上与if self.origin == 'camera'对应

        return transform_3dbox(box,transform_matrices["camera"])
    
    def get_masked_image(self,anno,frame):
        ...
        transform_matrices = { #这实际上就是camera
            'camera': np.eye(4, dtype=float),
            'lidar': frame_transforms.t_camera_lidar, #需要用这个lidar，因为现在是从lidar往camera转
            'radar': frame_transforms.t_camera_radar
        }
       
        img = frame_loader.image #H*W*3
        box = self.get_radarbox(anno,frame) #在origin（目前定义的是lidar）坐标系下的box
        center = box[0:3]
        box[3:6] = box[3:6] * 1.1
        corners_in_origin = boxes_to_corners_3d(box.reshape(-1,7))[0] #N*3,在origin（目前定义的是lidar）坐标系下的box的corners，
        #corners转换到图像坐标系下面
        corners_in_camera = transform_points(corners_in_origin,transform_matrices["lidar"]) 
        center_in_camera = transform_points(center.reshape(-1,3),transform_matrices["lidar"]) 

        new_corners_3d_hom = np.concatenate((corners_in_camera, np.ones((8, 1))), axis=1)
     
        corners_img = np.dot(new_corners_3d_hom, frame_transforms.camera_projection_matrix.T)
        corners_img = (corners_img[:, :2].T / corners_img[:, 2]).T #8*2 H*W

        img = apply_mask_with_smooth_edges(img, corners_img, sigma=20)
        depth = np.linalg.norm(center_in_camera)
        ...
        # plt.text(5, 5, str(anno["occluded"]),fontsize=20)
        # plt.text(10, 10, str(corners_img.tolist()))
        
        # plt.show()
        #---------------------------------------
        return img,depth 
        #按照anno的box，投影到图像，获取masked的图像
    ...
    def _get_frame_from_anno(self, anno):
        ...
        image,_ = self.get_masked_image(anno,frame)
        center = box[:3]
        dx,dy,dz = box[3:6]
        size = [dy,dx,dz]
        ry = -box[6] 
        ...
        return {"pc": pc, "3d_bbox": bb, "masked_image":image, 'meta': anno}
